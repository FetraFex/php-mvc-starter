$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.length) {
                for (let i = 0; i < data.length; i++) {
                    conversation += "<div class='membre conversation' data-id="+data[i].interlocinfo[0].idMembre+" style='display:flex;align-items:center;height:8vh'><img src='http://localhost/Rencontre/publics/image/default-profile.png' alt='' class='img img-responsive' style='width:65px;'><div style='line-height:0'><h4>" + data[i].interlocinfo[0].pseudo + "</h4><p>" + data[i].contenu + "</p></div></div>"
                }
